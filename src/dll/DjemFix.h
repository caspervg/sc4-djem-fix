...
class Fix final {
  ...
  private:
    ...
    static void __fastcall CalculateNormalsHook(cISTETerrainMap* terrain, void* edx,
                                                SC4Rect<std::int32_t>* dirtyVertices);
    static float __fastcall GetAltitudeHook(cISTETerrainMap* terrain, void* edx, float worldX, float worldZ);
