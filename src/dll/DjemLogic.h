...
namespace Djem
{
	...
	[[nodiscard]] CellDecision EvaluateCell(
		const CellHeights& heights,
		std::uint16_t flags,
		const Settings& settings) noexcept;

	[[nodiscard]] CellBounds ComputeAffectedCellBounds(
		const Rect& dirtyVertices,
		std::uint32_t maxCellX,
		std::uint32_t maxCellZ) noexcept;

	[[nodiscard]] bool TryMapWorldToCell(
		float worldX,
		float worldZ,
		float worldMaxX,
		float worldMaxZ,
		std::uint32_t maxCellX,
		std::uint32_t maxCellZ,
		WorldCell& result) noexcept;

	[[nodiscard]] float InterpolateFlippedCell(
		const CellHeights& heights,
		float localX,
	...
	[[nodiscard]] std::optional<std::uintptr_t> DecodeRelativeCallTarget(
		std::uintptr_t instructionAddress,
		const RelativeCallBytes& bytes) noexcept;

	[[nodiscard]] std::optional<RelativeCallBytes> EncodeRelativeCall(
		std::uintptr_t instructionAddress,
