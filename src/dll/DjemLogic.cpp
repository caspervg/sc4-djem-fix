...
namespace Djem {
CellDecision EvaluateCell(const CellHeights& heights, const std::uint16_t flags, const Settings& settings) noexcept {
    if ((flags & kCliffManagedFlag) != 0) {
        return {};
    }

    const auto [minHeight, maxHeight] = std::minmax({heights.h00, heights.h10, heights.h01, heights.h11});
    const bool currentlyFlipped = (flags & kInternalFlipFlag) != 0;

    if (maxHeight - minHeight < settings.minHeightDelta) {
        return {settings.clearNonCandidates && currentlyFlipped ? CellAction::SetNormal : CellAction::None, false};
    }
...
CellBounds ComputeAffectedCellBounds(const Rect& dirtyVertices, const std::uint32_t maxCellX,
    ...
    return {static_cast<std::uint32_t>(left), static_cast<std::uint32_t>(top), static_cast<std::uint32_t>(right),
            static_cast<std::uint32_t>(bottom), true};
}

bool TryMapWorldToCell(const float worldX, const float worldZ, const float worldMaxX, const float worldMaxZ,
                       const std::uint32_t maxCellX, const std::uint32_t maxCellZ, WorldCell& result) noexcept {
    if (!std::isfinite(worldX) || !std::isfinite(worldZ) || !std::isfinite(worldMaxX) || !std::isfinite(worldMaxZ) ||
        worldX < 0.0F || worldZ < 0.0F || worldX >= worldMaxX || worldZ >= worldMaxZ) {
        return false;
    }
...
float InterpolateFlippedCell(const CellHeights& heights, const float localX, const float localZ) noexcept {
    if (localX + localZ <= 1.0F) {
        return heights.h00 + (heights.h10 - heights.h00) * localX + (heights.h01 - heights.h00) * localZ;
    }

    return heights.h10 * (1.0F - localZ) + heights.h01 * (1.0F - localX) + heights.h11 * (localX + localZ - 1.0F);
}

std::optional<std::uintptr_t> DecodeRelativeCallTarget(const std::uintptr_t instructionAddress,
                                                       const RelativeCallBytes& bytes) noexcept {
    if (bytes[0] != 0xE8) {
        return std::nullopt;
    }

    const std::uint32_t encoded = static_cast<std::uint32_t>(bytes[1]) | (static_cast<std::uint32_t>(bytes[2]) << 8) |
...
std::optional<RelativeCallBytes> EncodeRelativeCall(const std::uintptr_t instructionAddress,
    ...
    return RelativeCallBytes{0xE8, static_cast<std::uint8_t>(encoded), static_cast<std::uint8_t>(encoded >> 8),
                             static_cast<std::uint8_t>(encoded >> 16), static_cast<std::uint8_t>(encoded >> 24)};
}
} // namespace Djem
