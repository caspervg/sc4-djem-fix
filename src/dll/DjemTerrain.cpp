...
#if !defined(_M_IX86)
#error SC4DjemFix terrain bindings require the 32-bit MSVC x86 ABI.
#endif
...
namespace
{
	...
	template <typename T>
	[[nodiscard]] T ReadField(const void* base, const std::uint32_t offset) noexcept
	{
	...
	[[nodiscard]] bool TryGetTerrainView(void* terrain, TerrainView& result) noexcept
	{
		...
		if (view.records == nullptr || view.cellCountX < 2 || view.cellCountZ < 2 ||
			view.rowStride < view.cellCountX ||
...
void Djem::TerrainProcessor::Configure(const Settings& settings) noexcept
{
	settings_ = settings;
}

void Djem::TerrainProcessor::SetActive(const bool active) noexcept
{
	active_ = active;
}

void Djem::TerrainProcessor::Apply(
	cISTETerrainMap* terrain,
	const SC4Rect<std::int32_t>* dirtyVertices)
{
	...
	const Rect dirtyRect{
		dirtyVertices->topLeftX,
		dirtyVertices->topLeftY,
		dirtyVertices->bottomRightX,
...
std::optional<float> Djem::TerrainProcessor::TryGetFlippedAltitude(
	cISTETerrainMap* terrain,
	const float worldX,
